...
export const createContact = async (contact: Contact) => {
  ...
  return await db.query('INSERT INTO contacts (name, phone) VALUES (?, ?) RETURNING id').get(contact.name, contact.phone) as Contact
}
...
export const updateContact = async (id: number, contact: Contact) => {
  ...
  return await db.run(`UPDATE contacts SET name = '${contact.name}', phone = '${contact.phone}'`)
}
